  ...
  db.oneOrNone('SELECT photo_id, upvotes, downvotes FROM votes WHERE photo_id = $1', id)
    .then((results) => {
      if(!results){
        ...
        db.one(`INSERT INTO votes(photo_id, ${direction}) VALUES($1, $2) RETURNING upvotes, downvotes`, [id, 1])
        .then((newresults1) => {
          console.log('1', newresults1);
          return newresults1;
        });
      } else {
        let newCount = results[direction] + 1;
        db.oneOrNone(`UPDATE votes SET ${direction}=$1 WHERE photo_id=$2 RETURNING upvotes, downvotes`, [newCount, id])
        .then((newresults2) => {
